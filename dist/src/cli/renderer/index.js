...
async function generateImages(options) {
    const { markdown, theme, fontSize, density, alignment, footerText, outputDir, debug } = options;
    ...
    try {
        ...
        const template = (0, templates_1.getTemplate)(theme);
        const strongTextColor = (0, templates_1.getDefaultStrongTextColor)(theme);
        const footerConfig = {
        ...
        if (debug) {
            const debugPath = path_1.default.join(outputDir, "debug-render.html");
            fs_1.default.writeFileSync(debugPath, htmlContent, "utf-8");
            console.log(`[DEBUG] 临时 HTML 已保存到: ${debugPath}`);
        }
        await page.setContent(htmlContent, { waitUntil: "networkidle0" });
        await page.setViewport({
            width: constants_1.CARD_CONFIG.previewWidth,
            height: constants_1.CARD_CONFIG.previewHeight * 10,
            deviceScaleFactor: constants_1.CARD_CONFIG.scale,
        });
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            const outputPath = path_1.default.join(outputDir, `${fileNamePrefix}-${i + 1}.png`);
            fs_1.default.writeFileSync(outputPath, screenshotBuffer);
...
function buildHtmlPage(options) {
    const { markdown, theme, fontSize, density, alignment, template, strongTextColor, footerConfig } = options;
    const fontSizePx = FONT_SIZE_MAP[fontSize];
    const densityConfig = DENSITY_CONFIG[density];
    const layout = (0, templates_1.getTemplateLayout)(theme);
    const codeBackground = (0, templates_1.getCodeBackground)(theme);
    const blockquoteColor = template.blockquoteColor ?? template.colors.accent;
    const pages = (0, pagination_1.calculatePages)(markdown, {
        density,
        fontSize,
        theme,
        strongTextColor,
    ...
    for (let i = 0; i < pages.length; i++) {
        ...
        const effectiveTemplate = (0, templates_1.getTemplate)(effectiveTheme);
        const effectiveColors = effectiveTemplate.colors;
        const effectiveStrongTextColor = effectiveTheme === "morandi" ? (0, templates_1.getDefaultStrongTextColor)("morandi") : strongTextColor;
        const effectiveLayout = (0, templates_1.getTemplateLayout)(effectiveTheme);
        const effectiveCodeBg = (0, templates_1.getCodeBackground)(effectiveTheme);
      ...
      <div class="card-page" style="
        width: ${constants_1.CARD_CONFIG.previewWidth}px;
        height: ${constants_1.CARD_CONFIG.previewHeight}px;
        position: relative;
        overflow: hidden;
        margin-bottom: 20px;
      ">
...
function renderCardContent(options) {
    const { pageContent, index, theme, template, colors, strongTextColor, layout, codeBackground, blockquoteColor, fontSizePx, densityConfig, alignment, footerConfig, } = options;
...
function renderDefaultCard(innerContent, colors, theme, fontSizePx, densityConfig) {
...
function renderCardFrameCard(innerContent, colors, template, fontSizePx, densityConfig) {
...
function renderSketchDecorationCard(innerContent, colors, template, fontSizePx, densityConfig) {
    return `
    <div class="card-content rounded-lg" style="
      color: ${colors.text};
      font-size: ${fontSizePx}px;
...
function renderSketchBackground(accentColor) {
    return `
...
function renderAppleNotesHeader(accentColor) {
    return `
...
function renderLennyCoverCard(markdown, colors, fontSizePx, alignment) {
...
function renderMarkdownContent(options) {
    const { content, colors, strongTextColor, codeBackground, blockquoteColor, fontSizePx, densityConfig, alignment, hasCustomHeader, footerConfig, } = options;
...
function renderFooter(text, accentColor) {
    return `
...
function extractCoverContent(markdown) {
    ...
    const topSectionLines = separatorIndex >= 0
        ? contentLines.slice(0, separatorIndex)
        : contentLines.slice(0, 2);
    const bottomSectionLines = separatorIndex >= 0
...
function getTitleFontSize(baseFontSize, title) {
