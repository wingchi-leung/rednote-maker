...
1.1 核心：智能分割算法
字数敏感型分页： 针对小红书 900×1200 比例，自动计算文本高度。当内容超过单张图片承载量（建议 800-1000 字/张）时，自动切分为「图 1、图 2...」。

自定义分页符： 在文档中单独一行写 `---` 会强制在此处分页，避免标题或句子被截断；分段后再按字数与语义继续分页。
...
Preview: 使用 aspect-ratio: 3 / 4 的容器，配合 drop-shadow 模拟纸张质感。支持两种可切换视图：**翻页模式**（单张卡片 + 上一页/下一页）、**竖排列表模式**（所有卡片竖排，区域可纵向滚动）。

4.3 主界面卡片化与编辑器行为 
- **布局**：主内容区背景 #F5F5F7；左「编辑器」、右「图片预览」为独立白卡片（圆角、阴影、间距），不再贴边或共用边框。
- **编辑器卡片**：标题「编辑器」；右上角「重置示例」按钮（恢复默认欢迎内容）、更多菜单图标（三点）。内容区仅纵向滚动。
- **预览卡片**：标题「图片预览」；视图切换（翻页 / 竖排列表）、翻页模式下显示分页控件。预览内容置于嵌套卡片内（浅底、圆角、细边框），形成层次感；竖排列表下多张卡片可纵向滚动浏览。
- **编辑器换行**：启用 CodeMirror `EditorView.lineWrapping`，长行在卡片宽度内自动换行；容器 `overflow-x-hidden`，无横向滚动。
